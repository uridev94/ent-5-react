...
const PokemonCard = ({ url }) => {
  ...
  return (
    <article
      style={{ border: `15px solid rgb(${colors[0]})` }}
    ...
    >
      <div
        ...
        className={`pokemonCard__back  ${pokemon?.types[0].type.name} `}
      >
        {" "}
      </div>
      ...
      <h3 style={{ color: `rgb(${colors[0]})` }} className="pokemonCard__name">
        {pokemon?.name}
      </h3>
        ...
        {pokemon?.stats.map(
          (stat) =>
            !stat.stat.name.includes("-") && (
              <li
                style={{ color: `rgb(${colors[7]})` }}
                className="pokemonCard__item"
                key={stat.stat.url}
              >
